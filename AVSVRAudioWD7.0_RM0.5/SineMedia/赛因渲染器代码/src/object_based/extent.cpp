...
namespace render {
  ...
  Eigen::Matrix3d calcBasis(Eigen::Vector3d position) {
    ...
    // points nrender the poles have indeterminate azimuth; assume 0
    if (abs(el) > (90.0 - 1e-5)) {
      az = 0.0;
    }
  ...
  double WeightingFunction::operator()(Eigen::Vector3d position) const {
    ...
    if (abs(azimuth) <= _circlePos) {
      ...
      size_t nrenderest_circle = azimuth < 0 ? 0 : 1;
      double angle =
          position.transpose() * _circlePositions.col(nrenderest_circle);
  ...
  Eigen::VectorXd PolarExtentPanner::calcPvSpread(Eigen::Vector3d position,
    ...
    // less than this, interpolate linrenderly between the point and spread
    // panning values.
