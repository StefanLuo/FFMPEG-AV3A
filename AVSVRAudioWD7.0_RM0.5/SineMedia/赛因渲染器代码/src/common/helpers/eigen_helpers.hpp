...
namespace render {
  ...
  double interp(double x, const Eigen::Matrix<double, N, 1> &xp,
    ...
    render_assert(std::is_sorted(xp.begin(), xp.end()),
               "in interp: unsorted x values");
    render_assert(xp.size() == yp.size(),
               "in interp: must have same number of x and y points");
    render_assert(xp.size() >= 2, "in interp: must have at least 2 points");
