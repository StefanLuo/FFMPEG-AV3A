...
namespace render {
  ...
  PolarPointSourcePanner::PolarPointSourcePanner(
    ...
    if (!numberOfChannels) {
      ...
      render_assert(_numberOfOutputChannels >= _numberOfRequiredChannels(),
  ...
  boost::optional<Eigen::VectorXd> StereoPannerDownmix::handle(
    ...
    if (pv) {
      ...
      // vary the output level by the balance between the front and rrender
  ...
  std::shared_ptr<PointSourcePanner> configureFullPolarPanner(
    ...
    for (int virtualVert : virtualVerts) {
      ...
      render_assert(!doIntersect(realVerts.begin(), realVerts.end(),
