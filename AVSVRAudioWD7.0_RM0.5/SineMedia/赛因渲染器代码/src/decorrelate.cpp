...
namespace render {
  ...
  /** @brief 设计all-pass随机相位FIR滤波器。
   *
   * @param decorrelator_id 设置随机种子参数，以获得不同的滤波器。Random seed, to obtain different filters.
   * @param 设置滤波器长度大小参数
   *
   * @return 返回值：滤波器参数
   */
  ...
  template <>
   RENDER_EXPORT std::vector<std::vector<float>> designDecorrelators<float>(
      Layout layout) {
