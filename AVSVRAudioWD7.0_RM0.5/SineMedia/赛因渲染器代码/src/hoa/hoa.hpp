...
namespace render {
  namespace hoa {
    ...
    inline norm_f_t &get_norm(NormType norm_type) {
      switch (norm_type) {
        ...
        case NormType::FuMa:
          return norm_FuMa;
        default:
          throw render::internal_error("invalid value for norm_type");
      }
    }
    ...
    Eigen::MatrixXd calc_Y_virt(Eigen::Matrix<double, Eigen::Dynamic, 3> points,
                                ...
                                NormT norm) {
      render_assert(n.size() == m.size(), "n and m must be the same size");
    ...
    /// AllRAD采用VBAP方式解码HOA信号，HOA解码单通道音箱的增益 gAllRAD(θ) = D yN(θ) ，VBAP gVBAP(θ) 提供了最优解D。 
    /// 我们通过积分来定义 AllRAD，该积分使用在所有方向 θ 上的积分来表达最小均方误差问题。
