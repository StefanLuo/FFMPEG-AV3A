...
class MonitoringAudioProcessor {
 public:
  ...
  std::vector<dsp::SampleIndex> vinterappoints_;

  void setInterp_points(dsp::SampleIndex index, const GainMatrix& direct,
      const GainMatrix& diffuse);
 ...
 private:
  ...
  dsp::GainInterpolator<dsp::LinearInterpMatrix> interp_;

  dsp::GainInterpolator<dsp::LinearInterpMatrix> interpdiffuse_;

  dsp::SampleIndex currentSampleIndex;
