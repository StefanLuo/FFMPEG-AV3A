...
struct BufferTraits {
  ...
  // number of channels in the buffer
	// static Eigen::Index channelCount(const Buffer& b) {return};
  // number of samples-per-channel in the buffer
	// static Eigen::Index size(const Buffer& b) {return};
  // pointer to the first sample of channel @a n for reading
	// static const SampleType* getChannel(const Buffer& b, std::size_t n) {return };
  // pointer to the first sample of channel @a n for writing
	// static SampleType* getChannel(Buffer& b, std::size_t n) {return &b[n]};
};
 ...
 public:
  ...
  void process(const InputBuffer& in, OutputBuffer& out) {
    ...
    while (sample < InTraits::size(in)) 
    {
      ...
      if (run_process) {
         // std::cout << " process fun " << input_buffer <<std::endl;
        process_func(input_buffer, output_buffer);
        samples_in_input = 0;
      }

      // check that we made progress
      render_assert(run_process || to_transfer > 0, "no progress made");
    }

     render_assert(sample == in.rows(), "processed more samples than expected");
     out = output_buffer;
  }
