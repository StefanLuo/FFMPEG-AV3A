...
#include "ext/THK_hrtf_database/hrtf_assets.h"
...
void SadieSphericalHarmonicHrir::CreateShHrirsFromAssets(const std::string &filename,
    ...
    if (thk::HrtfAssets::kAssetMap.find(filename) ==thk::HrtfAssets::kAssetMap.end()) {
        YggLogE("HRIR is nullptr");
        exit(-1);
    }

    auto& hrir_wav = thk::HrtfAssets::kAssetMap.at(filename);
...
SadieSphericalHarmonicHrir::SadieSphericalHarmonicHrir(size_t order, size_t sample_rate)
    ...
    static std::vector<std::string> sh_hrir_filenames = {"THK/Order1.hrir",
                                                         "THK/Order2.hrir",
                                                         "THK/Order3.hrir",
                                                         "THK/Order4.hrir",
                                                         "THK/Order5.hrir",
