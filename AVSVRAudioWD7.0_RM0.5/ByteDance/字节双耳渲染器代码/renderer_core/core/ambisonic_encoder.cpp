...
void AmbisonicEncoder::SetDopplerStatus(bool on, int sample_rate) {
    if (sample_rate <= 0)
        on = false;
    if (on == doppler_status_)
        return;
    // If closing Doppler effect, set the flag off before erasing delay buffer.
    if (!on) {
        doppler_status_.store(0);
        doppler_temp_buffer_.clear();
    }

    // If turning on Doppler effect, set the flag on after allocating delay buffer.
    if (on) {
        sample_rate_.store(sample_rate);
        doppler_temp_buffer_ = std::vector<float>(frames_per_buffer_);
        doppler_effect_ = std::make_shared<DelayLine<float>>(sample_rate);  ///< 1s maximum delay
        doppler_status_.store(1);
    }
}
...
void AmbisonicEncoder::Process(const float* input_buffer,
    ...
    if (doppler_status_.load()) {
        doppler_effect_->SetDelayInSamples(position.norm() / kSpeedOfSound * sample_rate_);
        doppler_effect_->Process(input_buffer, doppler_temp_buffer_.data(), num_frames);
    }
    ...
    RealSphericalHarmonics(ambisonic_order_, relative_dist.x(), relative_dist.y(), relative_dist.z(),
                           encoder_coeffs_.data(), true, true);
    for (int channel = 0; (channel < ambisonic_channels_ && channel < output_channels); ++channel) {
        ...
        if (ramp_length > 0) {
            vraudio_simd::MultiplyAndAccumulatePointwise(
                ...
                doppler_status_.load() ? doppler_temp_buffer_.data() : input_buffer, planar_output_buffer[channel]);
            vraudio_simd::ScalarMultiplyAndAccumulate(
                num_frames - ramp_length, ambisonic_ramps_.at(channel).target_value(),
                doppler_status_.load() ? doppler_temp_buffer_.data() + ramp_length : input_buffer + ramp_length,
...
void AmbisonicEncoder::Process(const float* input_buffer,
    ...
    for (int channel = 0; (channel < ambisonic_channels_ && channel < output_channels); ++channel) {
        ...
        if (ramp_length > 0) {
            vraudio_simd::MultiplyAndAccumulatePointwise(
                ramp_length, encoder_weights_.data(),
                doppler_status_.load() ? doppler_temp_buffer_.data() : input_buffer,
