...
public:
     ...
     * @param on
     */
    void SetDopplerStatus(bool on, int sample_rate = 0);

    bool GetDopplerStatus() {
        return doppler_status_;
    }
...
private:
    ...
    std::atomic_int doppler_status_;

    std::shared_ptr<DelayLine<float>> doppler_effect_;
    std::vector<float> doppler_temp_buffer_;
};
