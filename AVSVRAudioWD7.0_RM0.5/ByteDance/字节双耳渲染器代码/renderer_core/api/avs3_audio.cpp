...
audio_processor::audio_processor(::source_mode mode,
    ...
    switch (mode) {
        ...
        case SOURCE_SPATIALIZE: {
            ...
            direct_sound_encoder_->SetDopplerStatus(false);  ///< turn off Doppler by default
            break;
        }
    }
}
...
result audio_add_source(audio_context *ctx,
    ...
    return audio_add_source_with_orientation(ctx, source_mode, position, front, up, 0.1, source_id, is_async);
}
...
result audio_add_source_with_orientation(audio_context *ctx,
    ...
    if (radius < 0.1)
        radius = 0.1f;
    std::shared_ptr<avs3renderer::SoundSource> source = std::shared_ptr<avs3renderer::OmniSoundSource>(
            new avs3renderer::OmniSoundSource(source_position, source_front, source_up, avs3renderer::Spectrum(1.f), radius,
...
result audio_update_source_mode(audio_context *ctx,
    ...
    processor.object_ptr->source_mode = mode;
    return SUCCESS;
}
...
result audio_submit_source_buffer(audio_context *ctx,
    ...
    switch (processor->source_mode) {
        case SOURCE_SPATIALIZE: {
            avs3renderer::Vector3f source_listener_vec = source->Position() - ctx->listener->Position();
            float source_listener_dist = source_listener_vec.norm();
