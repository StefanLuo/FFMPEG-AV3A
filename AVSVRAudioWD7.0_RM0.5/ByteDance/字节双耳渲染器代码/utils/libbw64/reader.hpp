...
namespace bw64 {
  ...
  class Bw64Reader {
   public:
    ...
    template <typename ChunkType>
    std::vector<std::shared_ptr<ChunkType>> chunkAXml(
            const std::vector<std::shared_ptr<Chunk>>& chunks,
            uint32_t chunkId) const {
        std::vector<std::shared_ptr<ChunkType>> ret;
        for (auto &chunk : chunks) {
            if (chunk->id() == chunkId) {
                ret.push_back(std::static_pointer_cast<ChunkType>(chunk)) ;
            }
        }
        return ret;
    }
    ...
    std::shared_ptr<BxmlChunk> bxmlChunk() const {
        return chunk<BxmlChunk>(chunks_, utils::fourCC("bxml"));
    }

    std::vector<std::shared_ptr<AxmlChunk>> axmlChunks() const {
        return chunkAXml<AxmlChunk>(chunks_, utils::fourCC("axml"));
    }
