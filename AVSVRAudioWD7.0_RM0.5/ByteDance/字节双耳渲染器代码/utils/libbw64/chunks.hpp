...
namespace bw64 {
  ...
  class BxmlChunk : public Chunk {
  public:
      static uint32_t Id() { return utils::fourCC("bxml"); }

      BxmlChunk(const std::string& axml) {
          std::copy(axml.begin(), axml.end(), std::back_inserter(data_));
      }

      uint32_t id() const override { return BxmlChunk::Id(); }
      uint64_t size() const override { return data_.size(); }

      /*
       * @brief Write the BxmlChunk to a stream
       */
      void write(std::ostream& stream) const override {
          std::copy(data_.begin(), data_.end(),
                    std::ostreambuf_iterator<char>(stream));
