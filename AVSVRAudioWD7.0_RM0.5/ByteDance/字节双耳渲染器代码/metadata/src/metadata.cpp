...
std::vector<shared_ptr<ObjectFrame>> Metadata::parseObjectFrame(list<shared_ptr<AudioBlockFormat>> &block, double frame_duration) {
    ...
    shared_ptr<AudioBlockFormat> lastBlock;
    lastBlock = std::make_shared<AudioBlockFormat>();
    memcpy((void *) lastBlock.get(), (void *) audioBlockFormat[0].get(), sizeof(AudioBlockFormat));
    ...
    auto firstFrame = std::make_shared<ObjectFrame>();
    firstFrame->gain = lastBlock->gain;
    firstFrame->x = lastBlock->x;
    firstFrame->y = lastBlock->y;
    firstFrame->z = lastBlock->z;
    objectFrames.emplace_back(firstFrame);

    for (auto & currentBlock : audioBlockFormat) {
